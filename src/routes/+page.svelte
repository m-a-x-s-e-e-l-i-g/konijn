<script lang="ts">
	...
	function getImageProps(artwork: ArtworkMetadata) {
		const props: {
			src: string;
			alt: string;
			aspectRatio: number;
	...
	let fartSound = new Howl({
		src: ['/audio/weapons/fart.ogg'],
		onplay: () => {
			// Randomize the pitch between 0.5 and 1.5
			fartSound.rate(Math.random() * (1.5 - 0.5) + 0.5);
		}
	});
	const fartEquipSound = new Howl({ src: ['/audio/weapons/fart-equip.ogg'] });
	const pistolSound = new Howl({ src: ['/audio/weapons/pistol.ogg'] });
	const pistolEquipSound = new Howl({ src: ['/audio/weapons/pistol-equip.ogg'] });
	const weapons = [
	...
	onMount(() => {
		...
		tl.fromTo('#konijn', { y: -300 }, { y: 30, duration: 0.4, ease: 'power2.in' }) // jump
			.to('#konijn', { scaleY: 0.8, scaleX: 1.2, duration: 0.2, onComplete: emitBoing }, '0.3') // squash
			.to('#konijn', { y: -300, duration: 0.4, ease: 'power2.out' }) // fall
			.to('#konijn', { scale: 1, duration: 0.1 }, '0.5'); // stretch
...
<svelte:head>
	...
	<link
		href="https://fonts.googleapis.com/css2?family=Indie+Flower&family=Permanent+Marker&family=Rubik:ital,wght@0,300..900;1,300..900&display=swap"
		rel="stylesheet"
	/>
</svelte:head>
...
<div id="background">
	<div id="konijn-container">
		<svg
			...
			><path
				...
				data-name="right arm"
				class="cls-1"
				d="M369.79,487.72c-29.84,27.6-63,40.29-74.06,28.34s4.16-44,34-71.64,48.35-27.58,59.41-15.63S399.63,460.11,369.79,487.72Z"
				transform="translate(-77.63 -202.44)"
			/><ellipse id="body" class="cls-1" cx="150.5" cy="381.12" rx="148.5" ry="142" /><g
				id="left_foot"
				data-name="left foot"
				><circle class="cls-1" cx="99" cy="464.12" r="46" /><path
					class="cls-2"
					d="M161.63,623.29l1.18,11,.17,1.59a2.55,2.55,0,0,0,2.5,2.5,2.53,2.53,0,0,0,2.5-2.5l-1.18-11-.17-1.59a2.5,2.5,0,0,0-5,0Z"
					transform="translate(-77.63 -202.44)"
				/><path
					class="cls-2"
					d="M183,621.75l-1.82,10.46-.27,1.51a2.57,2.57,0,0,0,1.75,3.08,2.53,2.53,0,0,0,3.07-1.75l1.83-10.46.26-1.52A2.56,2.56,0,0,0,186.1,620a2.53,2.53,0,0,0-3.07,1.75Z"
					transform="translate(-77.63 -202.44)"
				/></g
			><g id="right_foot" data-name="right foot"
				><circle class="cls-1" cx="191" cy="469.12" r="46" /><path
					class="cls-2"
					d="M248.4,631l4,10.7.57,1.52a2.5,2.5,0,0,0,4.82-1.32l-4-10.71-.57-1.52a2.56,2.56,0,0,0-3.07-1.75A2.53,2.53,0,0,0,248.4,631Z"
					transform="translate(-77.63 -202.44)"
				/><path
					class="cls-2"
					d="M269.19,626.4l-.44,12.82-.07,1.85a2.5,2.5,0,0,0,5,0l.45-12.82.06-1.85a2.51,2.51,0,0,0-2.5-2.5,2.55,2.55,0,0,0-2.5,2.5Z"
					transform="translate(-77.63 -202.44)"
				/></g
			><path
				id="left_arm"
				data-name="left arm"
				class="cls-1"
				d="M105.53,503.42q5,5,10.62,9.87c30.84,26.48,47.91,23.86,58.52,11.5s10.74-29.76-20.1-56.24c-2-1.71-4-3.34-6-4.92A146.58,146.58,0,0,0,105.53,503.42Z"
				transform="translate(-77.63 -202.44)"
			/><g id="head"
				><path
					id="right_ear"
					data-name="right ear"
					class="cls-1"
					d="M303,282.75c-14.2,38.09-33.35,65.38-48.61,59.69s-16.11-41.18-1.91-79.27,34.58-50.45,49.84-44.76S317.22,244.66,303,282.75Z"
					transform="translate(-77.63 -202.44)"
				/><path
					id="left_ear"
					data-name="left ear"
					class="cls-1"
					d="M174.42,270.89c14.62,40.46,18.75,75.55,34.89,73.35s24.5-38.63,18.69-81.38-23.12-60.43-39.26-58.24S160.09,231.25,174.42,270.89Z"
					transform="translate(-77.63 -202.44)"
				/><ellipse
					id="head-2"
					data-name="head"
	...
	<div id="boing-counter">
		<span class="chewy-regular text-4xl">{$bounceCount}</span>
	</div>
</div>
<main class="pb-5">
	<div id="title" class="my-10 p-4 text-center">
		<h1 class="text-2xl sm:text-3xl md:text-4xl">
			...
			<span class="permanent-marker-regular">KO9</span>
	...
	<div id="image-container">
		{#each artworkCollection as artwork}
			<button
				class="konijn-artwork border-5 auto align-content-space-evenly m-0 w-[400px] border-black bg-black p-0 transition-transform duration-200 hover:scale-105 {isMobile
					? 'mobile-disabled'
					: ''}"
			...
			>
				<span
					class="absolute inline-block h-5 w-10 -translate-x-1/2 rounded-b bg-black text-center text-xs text-white"
				>
					#{artwork.id}
				</span>
	...
	<footer class="mt-10 flex w-full items-center justify-center">
...
<style>
	...
	#grass {
		...
		width: 100%;
		height: 100px;
		background: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><path d="M0 100 Q10 80 20 100 T40 100 T60 100 T80 100 T100 100 V100 H0 Z" fill="%2352a300"/></svg>')
			repeat-x;
	}
